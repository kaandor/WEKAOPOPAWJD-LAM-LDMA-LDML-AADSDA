...
async function showPreRollAd(slot, index, total) {
    ...
    label.textContent = `Anúncio ${index} de ${total}`;
    ...
    helper.textContent = "Se o anúncio não carregar, você poderá pular em instantes.";
    helper.style.color = "#d1d5db";
...
async function runPreRollAds(contentType) {
    let slots = [];
    if (contentType === "movie") {
        slots = MOVIE_AD_SLOTS;
    } else if (contentType === "series") {
        slots = SERIES_AD_SLOTS;
    }
    if (!slots || !slots.length) return;
    for (let i = 0; i < slots.length; i++) {
        await showPreRollAd(slots[i], i + 1, slots.length);
    }
}
    ...
    if (streamType === 'hls' || streamUrl.includes('.m3u8')) {
        ...
        const mp4Timeout = setTimeout(() => {
            ...
            if (proxyIndex < PROXY_LIST.length - 1) {
                ...
                showError(
                    "Erro: Tempo limite excedido. Você pode tentar abrir no player externo.",
                    createExternalAction(streamUrl)
                );
            }
...
function createExternalAction(streamUrl) {
    if (!streamUrl) return null;
    return {
        text: "Abrir em player externo (Modo Compatibilidade)",
        callback: () => {
            try {
                window.open(streamUrl, "_blank");
            } catch (err) {
                console.error("Erro ao abrir player externo:", err);
            }
        }
    };
}
...
export async function initPlayer() {
    ...
    try {
        ...
        const isSeriesContentForAds = type === "series" || type === "episode";
        const adType = type === "movie" ? "movie" : (isSeriesContentForAds ? "series" : null);
        if (adType) {
            await runPreRollAds(adType);
        }
