          ...
          if (p && typeof p.catch === 'function') {
            p.catch(e => {
             ...
             if (!url.includes("/stream-proxy") && url.startsWith("http")) {
                 ...
                 // Already proxied or not http. Fallback to HLS if not already tried?
                 // Or if native fails even with proxy, we might need HLS.js (if it's actually HLS content)
                 // But if it's MP4, HLS.js won't help.
                 console.error("Native play failed even with proxy or not applicable.");
             }
            });
          }
      }
