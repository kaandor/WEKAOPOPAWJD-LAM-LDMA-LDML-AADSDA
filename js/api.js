...
function escapeEmail(email) {
    ...
    return email.toLowerCase().replace(/\./g, ',').replace(/@/g, '_at_'); // Firebase não aceita . em chaves, mas aceita ,
}

function readSession() {
  const raw = localStorage.getItem(STORAGE_KEY);
  console.log("[API] Reading session from localStorage:", raw ? "Length: " + raw.length : "EMPTY");
  if (!raw) return null;
  try {
    const session = JSON.parse(raw);
    console.log("[API] Session parsed successfully. User ID:", session?.user?.id);
    return session;
  } catch (e) {
    console.error("[API] Error parsing session:", e);
    return null;
  }
}

function writeSession(session) {
  console.log("[API] Writing session to localStorage. User ID:", session?.user?.id);
...
function clearSession() {
  console.warn("[API] Clearing session from localStorage.");
...
async function request(method, path, body) {
  ...
  if (isClient) {
      ...
      if (method === "GET") {
          ...
          if (firebasePath) {
              console.log(`[Firebase] Path determined: ${firebasePath}`);
              let rawData = null;
  ...
  auth: {
    ...
    async login({ email, password, mac, key }) {
      try {
        if (isClientSideMode()) {
            if (USE_LOCAL_ONLY) {
                console.log("[Mock Auth] Login bypass (USE_LOCAL_ONLY=true)");
                const session = {
                    user: { id: "local_user", email: email, name: "Local User", plan: "premium", status: "active" },
                    tokens: { accessToken: "local-bypass-token" }
                };
                writeSession(session);
                return { ok: true, status: 200, data: { ...session } };
            }

            console.log(`[Firebase] Logging in for email: ${email}`);
            const emailKey = escapeEmail(email);
            console.log(`[Firebase] emailKey: ${emailKey}`);
            
            // Buscar usuário
            const fetchUrl = `${FIREBASE_DB_URL}/users/${emailKey}.json`;
            console.log(`[Firebase] Fetching user from: ${fetchUrl}`);
            const res = await fetch(fetchUrl);
        console.log(`[Firebase] User fetch response status: ${res.status} ${res.statusText}`);
        
        if (!res.ok) {
            console.error(`[Firebase] Fetch failed with status ${res.status}. Check CORS or Database URL.`);
            return { ok: false, status: res.status, data: { message: `Erro no banco: ${res.status}` } };
        }
        
        const user = await res.json();
            console.log(`[Firebase] User data result:`, user ? "Object found" : "Null/Not found");
            
            if (user && user.password === password) {
                console.log("[Firebase] Password matched.");
                ...
                console.log("[Firebase] Session object created:", JSON.stringify(session.user));
                writeSession(session);
                return { ok: true, status: 200, data: { ...session } };
            }
            
            if (user && user.password !== password) {
                console.warn("[Firebase] Password mismatch for user:", email);
            } else if (!user) {
                console.warn("[Firebase] User not found for email:", email);
            }
            
            return { ok: false, status: 401, data: { message: "E-mail ou senha incorretos." } };
        }
    ...
    async loginWithGithub() {
        console.log("[Mock Auth] Iniciando login simulado GitHub...");
        return new Promise(resolve => {
            setTimeout(() => {
                const user = { id: "github_local", name: "GitHub User", email: "github@klyx.app", plan: "premium", status: "active" };
                const session = { user, tokens: { accessToken: "gh_mock_token" }, provider: "github" };
                writeSession(session);
    ...
    async loginWithGoogle() {
        console.log("[Mock Auth] Iniciando login simulado Google...");
        return new Promise(resolve => {
            setTimeout(() => {
                const user = { id: "google_local", name: "Google User", email: "google@klyx.app", plan: "premium", status: "active" };
                const session = { user, tokens: { accessToken: "google_mock_token" }, provider: "google" };
                writeSession(session);
                resolve({ ok: true, status: 200, data: { ...session } });
            }, 100);
        });
