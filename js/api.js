...
// O sistema ir· carregar automaticamente a nova lista sem quebrar a lÛgica.
export const LIST_CONFIG = {
    MOVIES_FILE: "movies.json",  // Ex: "canaisbr05_filmes.json"
    SERIES_FILE: "series.json",  // Ex: "canaisbr05_series.json"
    EPISODES_PATH: "assets/data/episodes/", // Pasta dos episÛdios
    LIVE_FILE: "live.json"
};
// -----------------------------------------------------
...
function filterRestrictedContent(items) {
    ...
    const kidKeywords = ["animacao", "animation", "desenho", "infantil", "kids", "crianca", "crianÁa", "livre", "disney", "pixar", "fantasia", "fantasy", "familia", "family"];
...
function deduplicateMovies(items) {
    ...
    items.forEach(movie => {
        ...
        // Enrich Category for Smart Categorization (Kids/CrianÁa)
        // This ensures "CrianÁa" appears in the category dropdown if the movie matches safe keywords
        const keywordsSafe = ["animacao", "animation", "desenho", "infantil", "kids", "crianca", "crianÁa", "livre", "disney", "pixar", "fantasia", "fantasy", "familia", "family"];
        const combinedForCat = (title + " " + (movie.category || "")).toLowerCase();
        if (keywordsSafe.some(kw => combinedForCat.includes(kw))) {
             if (movie.category && !movie.category.includes("CrianÁa")) {
                 movie.category += " | CrianÁa";
             }
        }
...
export const api = {
  ...
  cloud: {
    ...
    // ATEN√á√ÉO: Substitua pela URL do seu projeto na Vercel se for diferente
    ...
    async syncDown() {
        console.log("‚òÅÔ∏è [Vercel DB] Syncing DOWN...");
        const session = readSession();
        ...
        try {
            ...
            if (data) {
                ...
                console.log("‚úÖ [Vercel DB] Sync Down Complete.");
                // Reload to apply changes if needed
                // window.location.reload(); 
            } else {
                console.log("‚ÑπÔ∏è [Vercel DB] No remote data found.");
            }
        } catch (e) {
            console.error("‚ùå [Vercel DB] Sync Down Failed:", e);
        }
    },

    // 2. Sync Up (Save to Vercel)
    async syncUp() {
        console.log("‚òÅÔ∏è [Vercel DB] Syncing UP...");
        const session = readSession();
        if (!session || !session.user) return;
        ...
        try {
            ...
            if (res.ok) {
                console.log("‚úÖ [Vercel DB] Sync Up Complete.");
            } else {
                console.error("‚ùå [Vercel DB] Sync Up Failed:", res.statusText);
            }
        } catch (e) {
            console.error("‚ùå [Vercel DB] Sync Up Error:", e);
        }
    },
  ...
  live: {
      async get(id) {
          try {
              ...
              if (!data) {
                  return { ok: false, data: { error: "Lista de canais indisponÌvel." } };
              }
    ...
    async getHome() { 
        try {
            ...
            const rails = {
                ...
                comedyMovies: getItems(allMovies, 100, m => (m.category || "").toLowerCase().includes("com√©dia")),
                actionMovies: getItems(allMovies, 100, m => (m.category || "").toLowerCase().includes("a√ß√£o"))
            };
    ...
    async getSeries() { 
        ...
        if (data && data.series) {
            ...
            data.series = series.map(s => {
                ...
                const keywordsSafe = ["animacao", "animation", "desenho", "infantil", "kids", "crianca", "crian√ßa", "livre", "disney", "pixar", "fantasia", "fantasy", "familia", "family"];
                const combinedForCat = (s.title + " " + (s.category || "")).toLowerCase();
                if (keywordsSafe.some(kw => combinedForCat.includes(kw))) {
                     if (s.category && !s.category.includes("Crian√ßa")) {
                         s.category += " | Crian√ßa";
                     }
                }
                return s;
    ...
    async getSeriesEpisodes(id) {
        ...
        try {
            ...
            if (!seriesRes.ok) {
                ...
                return { ok: false, data: { error: "S√©rie n√£o encontrada" } };
            }
            ...
            return { ok: false, data: { error: "Erro ao buscar epis√≥dios" } };
        }
    }
  },
  search: {
  ...
  profiles: {
      ...
      async delete(id) {
          ...
          if (profiles.length <= 1) {
              return { ok: false, data: { error: "VocÍ n„o pode excluir o ˙ltimo perfil." } };
          }
