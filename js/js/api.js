...
// --- CONFIGURAÃ‡ÃƒOO DA LISTA (LIST SWITCHING SYSTEM) ---
...
export const api = {
  ...
  cloud: {
    ...
    API_URL: (function() {
        ...
        return "https://klyx-db-server.vercel.app/api/db";
    })(),
    _syncTimer: null,
    ...
    async syncDown() {
        ...
        try {
            // Fetch User Data
            const controller = new AbortController(); setTimeout(() => controller.abort(), 5000); const res = await fetch(`${this.API_URL}?key=klyx:user:${session.user.id}`, { headers: { "Authorization": `Bearer ${session.tokens?.accessToken}` }, signal: controller.signal });
    ...
    async syncUp() {
        ...
        try {
            const controller = new AbortController(); setTimeout(() => controller.abort(), 5000); const res = await fetch(this.API_URL, { method: "POST", headers: { "Content-Type": "application/json", "Authorization": `Bearer ${session.tokens?.accessToken}` }, body: JSON.stringify({ key: `klyx:user:${session.user.id}`, value: data }), signal: controller.signal });
