...
export async function initPlayer() {
    ...
    try {
        ...
        const savedRes = await Promise.race([api.playback.getProgress(id), timeoutPromise]);
        const savedTime = savedRes && savedRes.ok ? savedRes.data.progress : 0;
        if (savedTime > 0) {
            console.log('[Player] Restoring progress: ' + savedTime + 's');
            video.currentTime = savedTime;
        }
    } catch (e) {
        console.warn('[Player] Failed to restore progress:', e);
    }
